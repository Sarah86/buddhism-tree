...
import flare02 from './assets/flare02.json';
...
function Tree(
  ...
  //Get the colors for more than one occurence
  const grouped_data = d3.group(descendants, (d) => d.data.name);
  let duplicatedValues = [];
  grouped_data.forEach((value, key, map) => {
    const valor = value.length > 1 ? key : null;
    if (valor)
...
function App() {
  ...
  const svg02 = useRef(null);
  useEffect(() => {
    if (svg.current) {
      svg.current.appendChild(chart);
    }
    if (svg02.current) {
      svg02.current.appendChild(chart02);
    }
  }, []);
  return (
    <div className="App">
